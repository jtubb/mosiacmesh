...
**REST API surface for the admin UI.** The new admin timeline view (PR-4) hydrates from REST endpoints, one module per resource under `mosaicmesh/api/`:
  ...
  - `/api/displays`  (GET / POST / DELETE) — `mosaicmesh/api/displays.py` (PR-12). First-class display-group CRUD. Each list entry has `displayID + clients[] + clientCount + onlineCount + scheduleCount`. No If-Match (groups have no `_serverVersion` — only `Display`'s internal playback state, which the timeline UI never PUTs). DELETE returns 409 + `refs:{clients, schedules}` when the group is in use; no force-delete.
...
- `mosaicmesh/` — server-side code split by responsibility:
  ...
  - `api/media.py`     — `GET /api/media` (lists media/server/{images,videos} + per-video durations); `DELETE /api/media` with body `{url}` (PR-16; 409+`refs` of playlist names when in use, 404 if missing, validates the URL is a `/media/server/{images|videos}/{filename}` path — rejects traversal attempts); `POST /upload/{dest}` (multipart upload routed to calibrate/image/video processors).
...
- `js/timeline/util/refetch-merge.js` — 412 conflict resolver. When `withRollback`'s apiFn throws `ApiError(status=412)`, this fetches the fresh entity, replaces the store slice, toasts *"X was updated by another admin"*. Wired from `util/optimistic.js` via `opts.conflictKind` + `opts.conflictId`. Handles `schedule`, `playlist`, and `profile` kinds; each is paired with a `refetch*` helper in `api.js` that prefers single-item GET and falls back to list+filter.
...
- **Display groups are first-class (PR-12).** The admin timeline renders one track per entry in `store.displayGroups` (hydrated from `GET /api/displays`), not per unique displayID in the client list. Groups with zero clients are visible — operators can pre-stage schedules for displays that aren't online yet. Create via the toolbar "+ Group" button (`mmToolbar.addDisplayGroup`); delete via right-click on a track header (`track-header-context-menu.js`). `store.displays` (the client list) is still hydrated alongside for status counts. Pre-PR-12 fallback: `timeline.js`'s `tracks` getter deduplicates `store.displays[].displayID` if `displayGroups` is empty, so the UI works against an old server (e.g., before the dev server restarts to pick up the new route).
- **Fleet actions are scope-aware (PR-13).** The toolbar's `.mm-fleet-scope` `<select>` binds to `mmToolbar.fleetScope` — empty string = all devices (legacy), any other value = a displayID. `fleetAction(which)` passes the current scope to `fireFleetAction(store, which, scope)` in `modals/fleet-confirm.js`. Scoped runs send `RUN_SCRIPT {displayID, script}`; all-device runs send `{all:true, script}`. Server-side `RUN_SCRIPT` handler in `mosaicmesh/websocket/legacy.py` has always supported both shapes — PR-13 just exposes them in the UI. The track-header right-click menu also surfaces the five actions (Login/Start/Stop/Reboot/Test) pre-scoped to that track's displayID for "I'm pointing at this track, run X on it" workflows.
- **Devices move between groups from the track-header popover (PR-14).** Left-click a track header → popover lists each client in the group with two dropdowns: **Profile** (PR-4c, sets the per-client `profileName` override) and **Group** (PR-14, moves the device to a different displayID). The Group dropdown is populated from `store.displayGroups`, so operators can't pick a non-existent group. Behind the scenes both go through `POST /api/discovery/configure {clientKey, displayID|profileName}`. PR-14 added server-side validation: an unknown `displayID` returns 404 (`"create it first"`) rather than silently setting the field to a string no group has — that footgun would make the device vanish from the timeline since `/api/displays` only enumerates known groups. Pre-PR-14 callers (`discovery.html` "Reconfigure") used the `{action:"reconfigure"}` action form which is unaffected.
- **Bulk move devices between groups (PR-15).** For 20+ device moves, the per-row Group dropdown is clunky. The track-header popover gains: a select-all checkbox at the top, per-row checkboxes, and a footer bulk-move bar (`.mm-thp-bulkbar`) that becomes visible when ≥1 row is selected. Apply fires `store.bulkAssignDevicesToDisplay` → `POST /api/discovery/configure {action:"bulk_assign", clientKeys, displayID}` server-side. Bulk endpoint is atomic — validates target group exists BEFORE iterating, returns `{moved:[...], missing:[...], movedCount}` (unknown keys are partial-success, not failure). Explicit operator moves (bulk or single) flip `client.autoConfigured = false` so the next REGISTER doesn't undo the choice via `auto_configure_client`. A drag-and-drop variant of the per-row move was prototyped during PR-15 but removed — dragging out of a positioned popover onto a target you can barely see was more cumbersome than the bulk-bar in practice.
