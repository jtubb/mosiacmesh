...
- **Bulk move + drag-and-drop devices between groups (PR-15).** For 20+ device moves, the per-row Group dropdown is clunky. The track-header popover now also has: a select-all checkbox at the top, per-row checkboxes, and a footer bulk-move bar (`.mm-thp-bulkbar`) that becomes visible when ≥1 row is selected. Apply fires `store.bulkAssignDevicesToDisplay` → `POST /api/discovery/configure {action:"bulk_assign", clientKeys, displayID}` server-side. Bulk endpoint is atomic — validates target group exists BEFORE iterating, returns `{moved:[...], missing:[...], movedCount}` (unknown keys are partial-success, not failure). Each popover row is also `draggable="true"` with drag kind `device-move` (in `drag/dragstate.js`); track headers accept the drop and fire the single-device `assignDeviceToDisplay`. Explicit operator moves (bulk or single) flip `client.autoConfigured = false` so the next REGISTER doesn't undo the choice via `auto_configure_client`.
