...
### Component 5 — Source URL: direct `file://` decode (obviates the local HTTP server)
The web client sets `<video>.src` to `http://127.0.0.1:8080/seg_*.mp4` (served today by an on-device **lighttpd**). `load()` **rewrites** this to `file:///var/mobile/Media/MosaicMeshCache/seg_*.mp4` and hands it to `AVPlayer` for **direct file decode — no local HTTP server in the path.** Mapping is the already-built, host-tested `mm_url_to_path` (strip the `http://127.0.0.1:8080/` prefix → prepend the cache dir, reject traversal). Non-localhost URLs (rare) pass through as-is to `AVPlayer` over http.

**Consequence — lighttpd becomes redundant for video.** Today the cache pipeline does two things: (1) **push** the rendered segment files onto the device (scp into the cache dir), and (2) **serve** them to the web `<video>` via lighttpd on `:8080`. The transplant reads the files directly, so **only the push half is still needed**; the serve half (lighttpd) is no longer on the video path. This also shifts the readiness signal from *"is lighttpd up"* to *"is the file present + complete on disk"* — which simplifies / partly retires the cache-capability machinery (the SSH `lighttpd`-alive probe, `cacheMode=lighttpd-localhost`, and the client `<video>`-error→`CACHE_LOCAL_FAIL` downgrade all lose their video rationale; see memory `cache-capability-is-client-local`). **Scope note:** v1 only changes the client (play `file://`); it does **not** rip out lighttpd or the probe — those stay running harmlessly. Actually retiring the lighttpd-serve layer (and simplifying the probe to a file-presence check) is a clean **server-side follow-up** once the transplant is validated fleet-wide, tracked separately.
