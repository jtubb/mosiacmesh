...
| T1.2 | Per-screen ffmpeg fan-out — O(N×V) processes per render | `render.py` (`_encode_group`) | 200 ffmpeg/render, ~500 s wall; calibrate-all over P playlists ≈ tens of minutes | **IMPLEMENTED, flag-gated OFF** (`MM_RENDER_FANIN`): SEGMENT mode now has a single-decode fan-in — one ffmpeg `split`s the decoded source into N branches (`build_ffmpeg_perspective_fanin_cmd`), batched by `MM_RENDER_FANIN_CAP` (default 8) so N libx264 encoders don't oversubscribe CPU. Equivalence is **golden-tested**: per-screen filtered frames are byte-identical to the per-process path (shared `_perspective_scale_vf`). INDIVIDUAL/FULL unchanged. Pending: enable flag + validate a real render on the wall, then flip default. |
