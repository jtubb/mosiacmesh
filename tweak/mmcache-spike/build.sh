...
rm -rf ~/mmcachespike && mkdir -p ~/mmcachespike
cp "$SRC"/Tweak.x "$SRC"/Makefile "$SRC"/mmcachespike.plist "$SRC"/control ~/mmcachespike/ 2>/dev/null
sed -i 's/\r$//' ~/mmcachespike/Makefile ~/mmcachespike/Tweak.x 2>/dev/null
cd ~/mmcachespike && make clean >/dev/null 2>&1 && make 2>&1 | grep -viE "tbd file|deprecated|Simulator" | tail -25
NM=/home/jtubb/theos/toolchain/linux/iphone/bin/llvm-nm
DY=$(find ~/mmcachespike -name '*.dylib' -not -path '*dSYM*' | head -1)
